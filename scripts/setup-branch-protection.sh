...
#
# Usage: ./scripts/setup-branch-protection.sh [--repo owner/repo] [--mode solo|multi-dev|team]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --mode)   MODE="$2"; shift 2 ;;
    --help|-h)
      echo "Usage: $0 [--repo owner/repo] [--mode solo|multi-dev|team]"
      echo ""
      echo "Configures branch protection rules for the main branch."
      echo ""
      echo "Options:"
      echo "  --repo owner/repo        Target repository (default: current repo)"
      echo "  --mode solo|multi-dev|team  Protection level (default: solo)"
      echo "    solo:      No force push, no deletions, 0 required reviews, direct push allowed"
      echo "    multi-dev: PRs required, 0 reviews (self-merge OK), linear history, no force push"
...
EOF
  ...
  echo "Error: --mode must be 'solo', 'multi-dev', or 'team'"
  exit 1
fi
